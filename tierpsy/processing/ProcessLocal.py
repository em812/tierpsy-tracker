...
from tierpsy.helper.misc import print_flush, replace_subdir
...
class ProcessLocal(object):
    def __init__(self, main_file, masks_dir, results_dir, tmp_mask_dir='',
        ...
        #EM: Add the possibility of running the command without mask_dir
        #    (only when we want to get features from summaries and fovslitting=False)
        if masks_dir is None:
            masks_dir = replace_subdir(results_dir, 'Results', 'MaskedVideos')
        ...
        #This flag should be optional in compress mode but true in track where
        #the src directory should be equal to the main_file
