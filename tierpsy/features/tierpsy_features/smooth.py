...
class SmoothedWorm():
    ...
    def _h_smooth_coords(self,
        ...
        EM: 1D interpolation, independently for x and y.
            Interpolate over a batch of frames with size s_win.
        '''
        ...
        # EM: if the dataset contains all nans, then do not smooth
        if dat_all[~np.isnan(dat_all)].size==0:
            return dat_all_s
